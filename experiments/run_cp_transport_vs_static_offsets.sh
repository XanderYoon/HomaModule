...
GBPS="${GBPS:-20}"
OFFSETS_US="${OFFSETS_US:-0,10,25,50}"
RESULTS_RUN_ROOT="$LOCAL_RESULTS_DIR/runs/transport_static_offsets"
...
LOG_DIR="$LOG_ROOT/cp_transport_static_offsets_${WORKLOAD}_${STAMP}"
...
log sync "Pushing updated static offset benchmark sources to $NODE0_ALIAS"
rsync -e "ssh -o StrictHostKeyChecking=no" -rtv \
    "$REPO_ROOT/util/cp_node.cc" \
    "$REPO_ROOT/util/cperf.py" \
    "$REPO_ROOT/util/cp_transport_vs_static_offsets" \
    "$NODE0_ALIAS:$REMOTE_REPO_DIR/util/"

ssh "$NODE0_ALIAS" "chmod 755 $REMOTE_REPO_DIR/util/cp_transport_vs_static_offsets"

log cleanup "Stopping stale benchmark processes on node0"
ssh "$NODE0_ALIAS" "
    set -euo pipefail
    ps -ef | awk '/[c]p_transport_vs_static_offsets/ {print \$2}' | xargs -r kill >/dev/null 2>&1 || true
...
log setup "Refreshing Homa runtime on node-0 through node-$((NUM_NODES-1)) before static offset benchmark"
...
log run "Launching cp_transport_vs_static_offsets on $NODE0_ALIAS"
ssh "$NODE0_ALIAS" "bash -lc 'cd $REMOTE_REPO_DIR/util && timeout 1800 ./cp_transport_vs_static_offsets -n $NUM_NODES --servers 1 -w $WORKLOAD -b $GBPS -s $RUN_SECONDS --offsets-us $OFFSETS_US -l $LOG_DIR'"

log fetch "Copying static offset benchmark results back to $LOCAL_RUN_DIR"
rsync -e "ssh -o StrictHostKeyChecking=no" -rtv \
    "$NODE0_ALIAS:$REMOTE_REPO_DIR/util/$LOG_DIR/" "$LOCAL_RUN_DIR/"
ln -sfn "$(basename "$LOCAL_RUN_DIR")" "$RESULTS_RUN_ROOT/latest"

log done "Static offset benchmark complete. Remote results are under $REMOTE_REPO_DIR/util/$LOG_DIR on $NODE0_ALIAS and local copies are under $LOCAL_RUN_DIR"
