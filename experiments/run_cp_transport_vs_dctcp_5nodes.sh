...
rsync -e "ssh -o StrictHostKeyChecking=no" -rtv \
    ...
    "$REPO_ROOT/util/cp_transport_vs_dctcp" \
...
log setup "Refreshing Homa runtime on node-0 through node-4 before transport cp_vs_tcp"
...
log done "transport cp_vs_tcp complete. Remote results are under $REMOTE_REPO_DIR/util/$LOG_DIR on $NODE0_ALIAS and local copies are under $LOCAL_RUN_DIR"
