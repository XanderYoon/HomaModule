...
log setup "Loading Homa on node-0 through node-4 for transport cp_basic"
...
log setup "Refreshing Homa runtime on node-0 through node-4 before transport cp_basic"
