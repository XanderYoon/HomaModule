...
usage() {
    cat <<'EOF'
Usage: run_cp_vs_tcp_10nodes.sh --workload WORKLOAD [options]
...
Notes:
  - Run ssh_setup_10nodes.sh first so node aliases and key-based SSH are configured.
...
log setup "Refreshing Homa runtime on node-0 through node-9 before cp_vs_tcp"
