...
GBPS="${GBPS:-20}"
RESULTS_RUN_ROOT="$LOCAL_RESULTS_DIR/runs/transport_combined"
...
LOG_DIR="$LOG_ROOT/cp_transport_combined_${WORKLOAD}_${STAMP}"
...
rsync -e "ssh -o StrictHostKeyChecking=no" -rtv \
    ...
    "$REPO_ROOT/util/cp_transport_vs_combined_dctcp" \
    "$NODE0_ALIAS:$REMOTE_REPO_DIR/util/"

ssh "$NODE0_ALIAS" "chmod 755 $REMOTE_REPO_DIR/util/cp_transport_vs_combined_dctcp"

log cleanup "Stopping stale benchmark processes on node0"
ssh "$NODE0_ALIAS" "
    set -euo pipefail
    ps -ef | awk '/[c]p_transport_vs_combined_dctcp/ {print \$2}' | xargs -r kill >/dev/null 2>&1 || true
    ps -ef | awk '/[c]p_transport_vs_dctcp/ {print \$2}' | xargs -r kill >/dev/null 2>&1 || true
...
log setup "Refreshing Homa runtime on node-0 through node-$((NUM_NODES-1)) before combined transport benchmark"
...
log run "Launching cp_transport_vs_combined_dctcp on $NODE0_ALIAS"
ssh "$NODE0_ALIAS" "bash -lc 'cd $REMOTE_REPO_DIR/util && timeout 1800 ./cp_transport_vs_combined_dctcp -n $NUM_NODES --servers 1 -w $WORKLOAD -b $GBPS -s $RUN_SECONDS -l $LOG_DIR'"

log fetch "Copying combined transport benchmark results back to $LOCAL_RUN_DIR"
rsync -e "ssh -o StrictHostKeyChecking=no" -rtv \
    "$NODE0_ALIAS:$REMOTE_REPO_DIR/util/$LOG_DIR/" "$LOCAL_RUN_DIR/"
ln -sfn "$(basename "$LOCAL_RUN_DIR")" "$RESULTS_RUN_ROOT/latest"

log done "Combined transport benchmark complete. Remote results are under $REMOTE_REPO_DIR/util/$LOG_DIR on $NODE0_ALIAS and local copies are under $LOCAL_RUN_DIR"
