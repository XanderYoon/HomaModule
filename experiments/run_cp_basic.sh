...
log report "Refreshing cp_basic_table.md from $LOCAL_RUN_DIR"
if ! python3 "$REPO_ROOT/experiments/generate_cp_basic.py" \
    "$LOCAL_RUN_DIR" --output "$REPO_ROOT/experiments/results/cp_basic_table.md"; then
    log warn "cp_basic summary markdown generation failed; benchmark artifacts were still fetched successfully"
fi
