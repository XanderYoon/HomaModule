...
def humanize_variant(variant_name):
    ...
    text = text.replace("_http2", " + HTTP/2")
    text = text.replace("_load_aware", " + Load-Aware")
    text = text.replace("_staggered", " + Staggered Scheduling")
    text = text.replace("_static", " + Static Offset")
    text = text.replace("_", " ")
    text = re.sub(r"\bp(\d+)\b", r"(pool \1)", text)
    text = re.sub(r"\bs(\d+)\b", r"(sessions \1)", text)
...
def sort_key(row):
    ...
    session_match = re.match(r"dctcp_http2_s(\d+)$", variant)
    if session_match:
        return (rank, "dctcp_http2", int(session_match.group(1)))
