...
MULTIPLEX_SESSIONS="${MULTIPLEX_SESSIONS:-4}"
POOL_SIZE="${POOL_SIZE:-1}"
TFO="${TFO:-false}"
LOG_ROOT="${LOG_ROOT:-logs}"
LOCAL_RESULTS_DIR="${LOCAL_RESULTS_DIR:-$REPO_ROOT/experiments/results}"
RESULTS_RUN_ROOT="$LOCAL_RESULTS_DIR/tuned_basic"
LOG_DIR=""
LOCAL_RUN_DIR=""
LOCAL_LOG_DIR=""
BASIC_OUTPUT=""
...
on_error() {
    local exit_code="$1"
    log warn "cp_tuned_basic run failed; fetching any partial logs to $LOCAL_RUN_DIR"
    fetch_partial_logs
    if [[ -n "$BASIC_OUTPUT" && -f "$BASIC_OUTPUT" ]]; then
        if [[ -s "$BASIC_OUTPUT" ]]; then
            log warn "Last cp_tuned_basic output lines"
...
usage() {
    cat <<'EOF'
Usage: run_cp_tuned_basic.sh [options]

Quick post-ssh-setup validation for Homa versus tuned DCTCP.
This follows run_cp_basic.sh closely, but the TCP-side comparison is a single
tuned DCTCP variant with HTTP/2 sessions and connection pooling.

Options:
  --seconds S           Duration for each cp_tuned_basic phase (default: 5)
  --timeout S           Timeout for the full cp_tuned_basic invocation
                        (default: auto, scaled from --seconds)
  --num-nodes N         Total nodes in the cluster (default: 5)
  --multiplex-sessions N    Tuned DCTCP HTTP/2 session count (default: 4)
  --pool-size N         Tuned DCTCP connection pool size (default: 1)
...
Environment overrides:
  ...
  MULTIPLEX_SESSIONS, POOL_SIZE, TFO, LOG_ROOT, LOCAL_RESULTS_DIR,
  LINK_MBPS, HOMA_MAX_NIC_QUEUE_NS, HOMA_RTT_BYTES, HOMA_GRANT_INCREMENT,
  HOMA_MAX_GSO_SIZE
EOF
}
...
if [[ -z "$TIMEOUT_SECONDS" ]]; then
    TIMEOUT_SECONDS=$((RUN_SECONDS * 24 + 120))
fi

STAMP="$(date +%Y%m%d%H%M%S)"
LOG_DIR="$LOG_ROOT/cp_tuned_basic_${STAMP}"
LOCAL_RUN_DIR="$RESULTS_RUN_ROOT/$(basename "$LOG_DIR")"
LOCAL_LOG_DIR="$LOCAL_RUN_DIR/logs"
mkdir -p "$RESULTS_RUN_ROOT"
trap 'on_error $?' ERR

log sync "Copying cp_tuned_basic benchmark to $NODE0_ALIAS"
rsync -e "ssh -o StrictHostKeyChecking=no" -rt \
    "$REPO_ROOT/util/cp_tuned_basic" \
    "$NODE0_ALIAS:$REMOTE_REPO_DIR/util/"
ssh "$NODE0_ALIAS" "chmod 755 $REMOTE_REPO_DIR/util/cp_tuned_basic"

log setup "Checking node0 runtime prerequisites on $NODE0_ALIAS"
ssh "$NODE0_ALIAS" "
    set -euo pipefail
    [[ -d $REMOTE_REPO_DIR/.git ]]
    [[ -x $REMOTE_REPO_DIR/util/cp_tuned_basic ]]
    [[ -x ~/bin/cp_node ]]
    [[ -x ~/bin/homa_prio ]]
    [[ -f ~/bin/homa.ko ]]
...
log run "Running cp_tuned_basic for Homa versus tuned DCTCP"
BASIC_OUTPUT="$LOCAL_RUN_DIR/basic_output.txt"
mkdir -p "$LOCAL_RUN_DIR"
ssh "$NODE0_ALIAS" "bash -lc 'cd $REMOTE_REPO_DIR/util && timeout $TIMEOUT_SECONDS ./cp_tuned_basic -n $NUM_NODES -s $RUN_SECONDS --tfo $TFO --multiplex-sessions $MULTIPLEX_SESSIONS --pool-size $POOL_SIZE -l $LOG_DIR'" \
    2>&1 | tee "$BASIC_OUTPUT"

log fetch "Copying cp_tuned_basic artifacts back to $LOCAL_RUN_DIR"
fetch_partial_logs
promote_pdfs
...
if find "$LOCAL_LOG_DIR" -maxdepth 1 -name 'node-*.log' -print -quit | grep -q .; then
    ...
    python3 "$REPO_ROOT/experiments/generate_cp_tuned_basic.py" \
        "$LOCAL_RUN_DIR" \
        --output "$SUMMARY_MD" \
        --title "cp_tuned_basic Summary"

    log summary "Saved summary table"
    sed -n '/^| Metric | Homa | Tuned DCTCP |$/,/^$/p' "$SUMMARY_MD"
else
...
log done "cp_tuned_basic run complete. Logs are in $LOCAL_RUN_DIR"
