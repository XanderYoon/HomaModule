...
def get_parser(description, usage, defaults = {}):
    ...
    parser.add_argument('--tcp-fastopen', dest='tcp_fastopen', type=boolean,
            ...
            '(default: false)')
    parser.add_argument('--tcp-multiplex', dest='tcp_multiplex', type=boolean,
            default=defaults['tcp_multiplex'],
            ...
            '(default: false)')
    parser.add_argument('--tcp-multiplex-sessions', type=int,
            dest='tcp_multiplex_sessions', metavar='count',
            default=defaults['tcp_multiplex_sessions'],
            help='Approximate HTTP/2 SETTINGS_MAX_CONCURRENT_STREAMS by '
            'limiting concurrent logical streams per shared TCP session '
            '(default: %d)'
            % (defaults['tcp_multiplex_sessions']))
...
def start_servers(r, options):
    ...
    if options.protocol == "homa":
        ...
        if getattr(options, "tcp_multiplex", False):
            command += " --tcp-multiplex"
        do_cmd(command, r)
    server_nodes = r
...
def run_experiment(name, clients, options):
    ...
    for client_rank, id in enumerate(clients):
        if options.protocol == "homa":
            ...
            if getattr(options, "tcp_multiplex", False):
                command += " --tcp-multiplex"
                sessions = getattr(options, "tcp_multiplex_sessions", 1)
                if sessions > 1:
                    command += " --tcp-multiplex-sessions %d" % (sessions)
            if getattr(options, "tcp_client_pooling", False) and \
                    not getattr(options, "tcp_multiplex", False):
            ...
            if (not getattr(options, "tcp_client_pooling", True)
                    and not getattr(options, "tcp_multiplex", False)):
