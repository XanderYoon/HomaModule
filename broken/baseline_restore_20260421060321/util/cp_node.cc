...
void print_help(const char *name)
{
	printf("Usage: cp_node [command]\n\n"
		...
		"                      port (default: %d)\n"
			...
			"    --tcp-multiplex       Approximate an HTTP/2-style multiplexed session by\n"
			"                      reusing shared TCP connections per server\n"
			"    --tcp-multiplex-sessions Approximate SETTINGS_MAX_CONCURRENT_STREAMS\n"
		...
		size_t *max_pending)
{
	...
	if (tcp_client_pooling) {
		...
		if (tcp_multiplex
				&& (connections[slot]->active_streams >= tcp_multiplex_sessions)) {
			return false;
		}
	} else {
...
void tcp_client::read(tcp_connection *connection, int pid)
{
	...
	int error = connection->read(epollet, [this, pid, &got_response]
		...
		uint64_t end_time = rdtsc();
		record(end_time, header);
		if (tcp_multiplex && (transport_slot >= 0)
