...
usage() {
    cat <<EOF
Usage:
  CLOUDLAB_USER=<cloudlab-user> bash ssh_setup/ssh_setup.sh

Common examples:
  Default safe MTU 1500:
    CLOUDLAB_USER=ARY bash ssh_setup/ssh_setup.sh

  Later, when the private experiment LAN supports jumbo frames:
    PAPER_MTU=3000 CLOUDLAB_USER=ARY bash ssh_setup/ssh_setup.sh
...
read_hosts() {
    ...
    [[ "${#HOSTS[@]}" -gt 0 ]] || {
        echo "Expected at least one host in $HOSTS_FILE" >&2
        exit 1
    }
}
...
# Auto-generated by ssh_setup/ssh_setup.sh
EOF
...
authorize_node0_key_on_peers() {
    ...
    log node0-key "Authorizing node0 SSH key on all configured nodes"
    for i in "${!HOSTS[@]}"; do
        ssh "node$i" "
            set -euo pipefail
            mkdir -p ~/.ssh
...
verify_node0_private_connectivity() {
    ...
    ssh "$NODE0_ALIAS" bash -s -- "${#HOSTS[@]}" <<'EOF'
set -euo pipefail
...
verify_cluster_setup() {
    log verify "Validating paper-style host tuning and runtime on all configured nodes"
    ssh "$NODE0_ALIAS" bash -s -- \
        "${#HOSTS[@]}" \
        "$PAPER_MODE" \
        "$PAPER_EXPECT_XL170" \
...
- node0 fanout SSH using private aliases for all configured hosts
